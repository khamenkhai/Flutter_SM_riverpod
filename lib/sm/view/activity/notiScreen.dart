...
class NotiScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context,WidgetRef ref) {
    return Scaffold(
      ...
      body: ref.watch(getNotificationsOfUserProvider).when(data: (notis){
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    //profile image
                    CircleAvatar(
                      backgroundImage: NetworkImage("${notis[index].senderProfile}"),
                    ),
                    SizedBox(width: 15),
                    Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            Text(notis[index].senderName.toString(),style:TextStyle(fontWeight: FontWeight.bold)),
                            SizedBox(width: 10),
                            Text(notis[index].notiMessage.toString()),
                          ],
                        ),
                        SizedBox(height: 5),
                        //noti time
                        Text("${timeago.format(notis[index].dateTime!)}",style: TextStyle(color: Colors.grey))
                      ],
                    ),
                  ],
                ),),
